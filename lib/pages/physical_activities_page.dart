...
class PhysicalActivitiesPage extends StatefulWidget {
  ...
  @override
  State<PhysicalActivitiesPage> createState() => _PhysicalActivitiesPageState();
}
...
class _PhysicalActivitiesPageState extends State<PhysicalActivitiesPage> {
  ...
  final Set<String> _patientActivityIds = {}; // Lưu physicalActivityId của bệnh nhân
  ...
  Future<void> _fetchAll() async {
    ...
    try {
      ...
      final all = await _service.fetchPhysicalActivities();
      final patient = await _service.getPatientPhysicalActivities(patientProfileId);
      final Set<String> patientIds = {};
      final Set<int> selectedIndexes = {};
      for (final item in patient) {
        if (item['physicalActivityId'] != null) {
          patientIds.add(item['physicalActivityId']);
        }
      }
      // Debug log: print all ids
      print('All physical activities ids: ' + all.map((e) => e['id']).toList().toString());
      print('Patient physicalActivityIds: ' + patientIds.toString());
      for (int i = 0; i < all.length; i++) {
        if (patientIds.contains(all[i]['id'])) {
          print('MATCH: activity id ${all[i]['id']} is in patient physicalActivityIds');
  ...
  IconData getActivityIcon(String name) {
    ...
    if (lower.contains('aerobics')) return Icons.music_note;
    if (lower.contains('archery') || lower.contains('bắn cung')) return Icons.sports_handball;
    if (lower.contains('cầu lông')) return Icons.sports_tennis;
    if (lower.contains('leo cầu thang')) return Icons.stairs;
    if (lower.contains('đạp xe')) return Icons.directions_bike;
    if (lower.contains('golf')) return Icons.sports_golf;
    if (lower.contains('gym')) return Icons.fitness_center;
    if (lower.contains('leo núi')) return Icons.terrain;
    if (lower.contains('chạy')) return Icons.directions_run;
    if (lower.contains('nhảy dây')) return Icons.sports_kabaddi;
    return Icons.sports;
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Hoạt động thể chất')),
      body: _loading
          ? const Center(child: CircularProgressIndicator())
          : Column(
              children: [
                // Progress bar and selected count
                Container(
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 8),
                        Text(
                          'Tuyệt vời! Bạn đã chọn ${_selectedIndexes.length} bài tập',
                ...
                Expanded(
                  child: GridView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final isSaved = _patientActivityIds.contains(activity['id']);
                                      ...
                                      Container(
                                        ...
                                        child: Row(
                                          ...
                                          children: const [
                                            ...
                                            SizedBox(width: 4),
                                            Text('Hằng ngày', style: TextStyle(color: Colors.green, fontWeight: FontWeight.w600)),
                                          ],
                                        ),
                                      )
                                    else if (isSelected)
                ...
                Padding(
                  ...
                  child: SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      onPressed: _selectedIndexes.isEmpty ? null : () async {
                        ...
                        try {
                          ...
                          final selected = _selectedIndexes.map((i) => {
                            'physicalActivityId': _activities[i]['id'],
                            'preferenceLevel': 'Daily',
                          }).toList();
                          await _service.savePatientPhysicalActivities(patientProfileId, selected);
                          if (mounted) {
                      ...
                      child: Text(
                        'Tiếp tục với ${_selectedIndexes.length} bài tập',
